...
pussy = (Write.Input("""\n┌────(Server Cloner)-[Want To Install Module? (Y/N)]
...
if pussy.lower() == 'y':
    ...
    Write.Print("""\n┌────(Server Cloner)-[+] Requirements Installed Successfully!""", Colors.green_to_yellow, interval=0.02)

elif pussy.lower() == 'n':
    
    Write.Print("""\n┌────(Server Cloner)- [-] Skipping Requirements Installation...""", Colors.green_to_yellow, interval=0.02)
else:
    Write.Print("""\n┌────(Server Cloner)- [-] Invalid Choice. Please enter 'Y' or 'N'""", Colors.green_to_yellow, interval=0.02)
...
Anime.Fade(Center.Center(join), Colors.rainbow, Colorate.Vertical, interval=0.080, enter=True)
ascii_art = r"""
...
"""
gradient_colors = Colorate.Vertical(Colors.green_to_yellow, ascii_art)
...
"""
    print(menu)
    choice = await get_input(f"{Fore.CYAN}Enter your choice (1-5): {Style.RESET_ALL}")
    return choice
